#!/bin/bash

source etc/facri.conf

source bin/facri-friends.sh
#source bin/displaytime

# FEEDS_OUTPUT=$FACRI_OUTPUT/feeds/friends
# 
# friends_ids=$($FACRI_ROOT/bin/facri-friends.sh)
# 
# me_id=$($FBCMD whoami | awk '{print $1}')

start=$(date +%s)

cd $FEEDS_OUTPUT
# total_users=$(echo $friends_ids | wc -w)
# echo "$total_users users"
# declare user_counter=0
# for u in $friends_ids; do
#    TARGET=$FEEDS_OUTPUT/$u
#    user_counter=$((user_counter+1))
#    [ ! -d $TARGET ] && echo "user $u [$user_counter/$total_users] not existant: go further." && continue
# 
#    echo -n "user $u [$user_counter/$total_users]: "
#    cd $TARGET
#    # all posts_ids
# #   posts_ids=$(cat posts*.json | jq '.posts[] | .post_id')
# 
#    # posts_ids with non-zero comments
#    posts_ids=$(cat posts*.json | jq '.posts[] | select((.comment_info | .comment_count != "0") or (.likes | .count !="0") or (.share_count!="0")) | .post_id')
# #   echo posts_ids $posts_ids
#    tot_post_ids=$(echo $posts_ids | wc -w)
#    echo $tot_post_ids posts
#    for c in ${posts_ids}; do
#       c_id=$(echo $c | cut -d"_" -f2 | sed 's/"//g')
#       echo -n -e "\t$c_id: "
#       [ -f "${c_id}_comments.json" ] && echo "existant, skip." && continue
#       s=$($FBCMD PCOMMENTS $c_id 150) 
#       [ $s ] && echo "ok." || echo "empty."
#    done
#    echo ""
# done



function getComments() {
  local feeds_output=$1
  shift 
  local ids=$@
  local total_users=$(echo $ids | wc -w)
  echo "$total_users users"
  local user_counter=0
  
  for u in $ids; do
    local target=${feeds_output}/$u
    local user_counter=$((user_counter+1))
    [ ! -d $target ] && echo "user $u [$user_counter/$total_users] not existant: go further." && continue

    echo -n "user $u [$user_counter/$total_users]: "
    cd $target
    # all posts_ids
    #   posts_ids=$(cat posts*.json | jq '.posts[] | .post_id')

    # posts_ids with non-zero comments
    local posts_ids=$(cat posts*.json | jq '.posts[] | select((.comment_info | .comment_count != "0") or (.likes | .count !="0") or (.share_count!="0")) | .post_id')
    #   echo posts_ids $posts_ids
    local tot_post_ids=$(echo $posts_ids | wc -w)
    echo $tot_post_ids posts
    for c in ${posts_ids}; do
      local c_id=$(echo $c | cut -d"_" -f2 | sed 's/"//g')
      echo -n -e "\t$c_id: "
      [ -f "${c_id}_comments.json" ] && echo "existant, skip." && continue
      local s=$($FBCMD PCOMMENTS $c_id 150) 
      [ $s ] && echo "ok." || echo "empty."
    done
    echo ""
  done
}

friends_feeds_output="${FACRI_OUTPUT}/feeds/friends"
# friends_ids=$(${FACRI_ROOT}/bin/facri-friends.sh)
friends_ids=$(getFriends)
getComments ${friends_feeds_output} $friends_ids

me_feeds_output="${FACRI_OUTPUT}/feeds/me"
me_id=$($FBCMD whoami | awk '{print $1}')
getComments ${me_feeds_output} ${me_id}

end=$(date +%s)
r=$(echo "$end - $start" | bc)
echo -e "\n\ndone in $($FACRI_ROOT/bin/displaytime $r)\n"